...
# --- Inventory Screen
# ========================
class InventorySystem(MenuScreen):
    def __init__(self):
        ...
        self.scroll_y = 0
        self.max_scroll = 0
        self.tabs = ["All", "potion", "ingredient", "weapon", "armor", "material", "accessory", "misc"]
        self.active_tab = "All"
        self.tab_rects = []
        self.visible_slot_map = []
        self.inventory_area = (config.INV_START_X, config.INV_START_Y, config.INV_WIDTH, config.INV_HEIGHT)
    ...
    def draw(self, surface):
        ...
        surface, self.total_height, self.max_scroll, self.scroll_y, self.visible_slot_map = cont.draw_inventory_slots(self.database, self.inventory_area, self.active_tab, self.dragging_item, self.dragging_from, self.hover_slot, self.scroll_y, config.INV_COLS, surface)

        #self.draw_inventory_tabs(surface)
        self.tab_rects = gadgets.draw_inventory_tabs(surface, self.tabs, self.active_tab)
        gadgets.draw_equipment_slots(self.database, self.dragging_item, self.dragging_from, self.hover_slot, surface)
